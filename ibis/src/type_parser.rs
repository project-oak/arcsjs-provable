...
pub fn read_type(input: &str) -> Type {
  ...
  if input != "" {
    todo!("Did not reach end of input. Read {:?}. Left over {}", ty, input);
  }
  ty
}
...
mod tests {
    ...
    fn read_a_simple_type_name() {
        assert_eq!(read_type("Type"), Type { name: "Type", args: vec![] });
    }

    #[test]
    fn read_a_type_with_arguments() {
        assert_eq!(read_type("Type(a, b)"), Type { name: "Type", args: vec![
            Type { name: "a", args: vec![] },
            Type { name: "b", args: vec![] },
        ] });
    }

    #[test]
    fn read_a_type_with_nested_arguments() {
        assert_eq!(read_type("Type(a(c), b)"), Type { name: "Type", args: vec![
            Type { name: "a", args: vec![
                Type { name: "c", args: vec![] }
            ] },
            Type { name: "b", args: vec![] },
        ] });
    }
